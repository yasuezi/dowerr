...
async function sendTransactionLog(data) {
    try {
        ...
        const escapedUsername = username ? `🔖 Username: ${escapeMarkdownV2('@' + username)}` : '🔖 Tidak ada username';

        const logMessage = escapeMarkdownV2(`
╭─〔 TRANSAKSI BERHASIL 〕────────╮
│ 💰 Jumlah: Rp ${amount}
│ 📱 Nomor: ${phoneNumber}
│ 🧾 Referensi: ${reference}
...
╰───────────────────────────────╯`);
        ...
        console.log(`Transaction log sent to group for user ${userId}`);
    } catch (error) {
